...
Person::Person()
{mpz_inits( public_key, recive, temp, NULL);}
...
{
	//告诉第三方自己的成绩
	basic.Send_Message("Third_1\\Bob_Grade.txt", grade);
	//从第三方获取公钥、私钥
...
bool Person::Judge()
{
	...
	{
		cout << "  ║解得Alcie发送明文与我方成绩相同║\n";
		cout << "  ║    所以，Alcie成绩和自己的    ║\n";
		cout << "  ║        = = = 相同 = = =       ║\n";
		cout << "  ╰═══════════════════════════════╯\n\n";
		return true;
	}
		cout << "  ║解得Alice发送明文与我方成绩不同║\n";
		cout << "  ║    所以，Alcie成绩和自己的    ║\n";
		cout << "  ║       * * * 不相同 * * *      ║\n";
		cout << "  ╰═══════════════════════════════╯\n\n";
		return true;
...
void Person::Game()
{
	...
	while (1)
	{
		...
		cout << "    ======发送完成======\n\n";
		basic.Send_Message("Third_1\\Bob_Send.txt", send2);
		//Sleep(2000);
		system("pause");

		//接收第三方发送:是否作弊的通知/Alice成绩密文
		if (Check() == false)continue;
		Vision('r');
		while (1)
		{
	...
	//通知Alice、第三方，已退出
	remove("Third_1\\Bob_Grade.txt"),remove("Third_1\\Bob_Send.txt");
