...
Person::Person() { mpz_inits(rAB, rAC, r3, recive, commit, send, temp, NULL); }
...
bool Person::Init()
{
	...
	cout << "   > Step 1：发送数 rAB 给Bob\n";
	cout << "     双方约定 rAB 为计算Pedersen承诺时使用的参数\n";
	cout << "     参数 rAB 实际值为：";
	gmp_printf(" %Zd\n", rAB);
	cout << "   > 交换过程如下\n";
	basic.Send_Agreement_Number("Alice_3", "Bob_3", rAB);
	system("pause");
	//从第三方获取rAC
	if (Check() == false) return false;
	Vision('0');
	cout << "   > Step 2：等待第三方发送约定参数rAC\n";
	cout << "   > 交换过程如下\n";
	basic.Recieve_Agreement_Number("Alice_3", "Third_3", rAC);
	Sleep(2000);
	//从第三方获取r3
	if (Check() == false) return false;
	Vision('0');
	...
	cout << "   > 交换过程如下\n";
	basic.Recieve_Agreement_Number("Alice_3", "Third_3", r3);
	return true;
}
...
void Person::Game()
{
	...
	while (1)
	{
		...
		basic.Send_Message("Third_3\\Alice_Send.txt", send);
		//Sleep(3000);
		system("pause");
	...
	//通知Bob、第三方，已退出
	remove("Bob_3\\rAB.txt"),remove("Third_3\\Alice_Send.txt");
	basic.Send_Message("Bob_3\\End.txt", 0);
	basic.Send_Message("Third_3\\End.txt", 0);
}
...
void Vision(char choose)
{
	...
	case 'g':
		...
		break;
	case 's':
		cout << "   > 现在即将计算Pedersen承诺: C = (g^rAB)^SA mod p\n"; 
		cout << "     再加上与第三方约定随机数 rAC 后，发送给第三方\n"; Sleep(1000);
		break;
	case 'r':
		cout << "      等待第三方发送密文…\n";
		break;
